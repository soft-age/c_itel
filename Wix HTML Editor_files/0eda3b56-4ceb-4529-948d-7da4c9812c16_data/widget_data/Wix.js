(function (container) {
    ...
    _w.PostMessageDispatcher = (function(){
        return {
    ...
    var Wix = {
        ...
        /**
         * Wix errors that are sent to the developer
         */
        Error: {
            WIX_ERROR: 'Wix error'
        },
        ...
        Dashboard: {
			 ...
			 * Function openModal
             *
             * Opens a new modal window inside the dasboard, including a blocking light box. The modal is a TPA Widget which is not positioned inside the dashboard layout but floats on top of it.
			 *
			 * @since SDK 1.27.0
			 ...
			 *
			 * Closes the dashboard app's modal window.
			 * This function can be used from a modal scope only.
			 *
			 * @since SDK 1.27.0
			 * @param (Object)
			 */
		...
		Activities: {
			...
			postActivity: function(activity, onSuccess, onFailure) {
				...
				_w.sendMessageInternal2(_w.MessageTypes.POST_ACTIVITY, args, onComplete);
			},
			/**
			...
			getUserSessionToken:function(callback) {
				_w.sendMessageInternal2(_w.MessageTypes.GET_USER_SESSION, null, callback);
			}
		},
        ...
        PubSub: {
            ...
            publish:function(eventKey, data, isPersistent){
                ...
                _w.sendMessageInternal2(_w.MessageTypes.PUBLISH, {
        ...
        Contacts: {
            ...
            getContacts: function (onSuccess, onFailure) {
                ...
                var onComplete = function onComplete(response) {
					if (!response.error) {
						var cursor = new Wix.WixDataCursor(_w.PostMessageDispatcher, _w.MessageTypes.GET_CONTACTS, response.data.results);
                        cursor.setNextCursor(response.data.nextCursor);
                        cursor.setPreviousCursor(response.data.previousCursor);
						onSuccess(cursor);
                    } else {
						onFailure(Wix.Error.WIX_ERROR);
					}
                };

                _w.sendMessageInternal2(_w.MessageTypes.GET_CONTACTS, undefined, onComplete);
            },
            ...
            getContactById: function (id, onSuccess, onFailure) {
                ...
                _w.sendMessageInternal2(_w.MessageTypes.GET_CONTACT_BY_ID, args, onComplete);
            }
        },
